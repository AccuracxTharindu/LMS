            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
					...
					<?php
						...
						//$status = "borrowed";
						//$sqll = mysqli_query($con,"SELECT COUNT(`user_id`) as `total` FROM borrow_book WHERE `user_id` = '$row['user_id']' and `borrowed_status` = '$status'");
						$no = $row['user_id'];
						$sqll = mysqli_query($con,"SELECT COUNT(`user_id`) as `total` FROM borrow_book WHERE `user_id` = '$no' and `borrowed_status` = 'borrowed'");
						
						$roww = mysqli_fetch_array($sqll);
						
						$sqlll = mysqli_query($con,"SELECT COUNT(`user_id`) as `totall` FROM borrow_book WHERE `user_id` = '$no' and `borrowed_status` = 'returned'");
					...
					<h2>
					No of Borrowed Books : <span style="color:maroon;"><?php echo $roww['total']; ?></span>
					</h2>
					...
					?>
					No of Returned Books : <span style="color:maroon;"><?php echo $kk; ?></span>
					</h2>
						...
						<table class="table table-bordered">
							...
							<?php 
								if (isset($_POST['borrow'])){
									...
									$date_borrowed =$_POST['date_borrowed'];
									$due_date =$_POST['due_date'];
									echo $user_id."-->".$book_id."-->".$date_borrowed."-->".$due_date;
									...
									/*
									mysqli_query($con,"INSERT INTO borrow_book(user_id,book_id,date_borrowed,due_date,borrowed_status) 						VALUES('$user_id','$book_id','$date_borrowed','$due_date','borrowed')") or die (mysqli_error());
									*/
									
									
									
$sql = "INSERT INTO borrow_book(user_id,book_id,date_borrowed,due_date,borrowed_status) VALUES('$user_id','$book_id','$date_borrowed','$due_date','borrowed')";
                if(mysqli_query($con, $sql)){
                //echo "klklklv";
                    echo "<script>alert('Borrow Book successfully added!');</script>";
                } else{
                    //echo "ERROR: Could not able to execute $sql. " . mysqli_error($con);
                    echo "<script>alert('ERROR added!');</script>";
                }									
